"iclust.diagram" <- 
function(ic,labels=NULL,short=FALSE,digits=2,cex=NULL,min.size=NULL,e.size=.6, main="ICLUST diagram") {
...
num <- nrow(clusters)
num.var <- num+1
if(is.null(cex)) cex <- min(20/num.var,1)
...
length.labels <- max(fixed$max.len * .12 * cex,.25*cex)
##
...
for(clust in 1:nc) {
 ...
 "dia.cluster" <- 
 function(x, y = NULL, cluster, link=NULL, digits=2,cex = cex,e.size=.6,xlim=c(0,1),ylim=c(0,1), ...) {
	...
	text(x,y, substitute(list(beta) == list(b), list(b=round(cluster[1,"beta"],digits))),cex=cex,pos=1) 
     ...
     bottom <- c(x,y-xs)
     center <- c(x,y)
    dia.cluster <- list(left=left,right=right,top=top,bottom=bottom,center=center,link=link)
     }
  
 "dia.cluster2" <- 
 function(x, y = NULL, cluster, link=NULL, digits=2,cex = cex,e.size=.5,xlim=c(0,1),ylim=c(0,1), ...) {
    text(x,y, rownames(cluster),cex=cex)
	 vert <- cex*.3
     xs <- dia.ellipse1(x,y,xlim=xlim,ylim=ylim,e.size=e.size,...)
     left <- c(x-xs,y)
     right <- c(x+xs,y)
     ...
     bottom <- c(x,y-xs)
     center <- c(x,y)
    dia.cluster2 <- list(left=left,right=right,top=top,bottom=bottom,center=center,link=link)
     }
  ...
  #down is a recursive function that draws the complete cluster structure
   ...
   function(clusters,head,x,y,old.head = NULL,old.loc=NULL,digits,cex,limx,limy,min.size=1,e.size=.6,...) {
           ...
           if(size > min.size) {c.loc <- dia.cluster(head+shift,y,cluster,link=link,digits=digits,cex=cex,e.size=e.size) } else {c.loc <- dia.cluster2(head+2,y,cluster,link=link,digits=digits,cex=cex,e.size=e.size*.6)}
           ...
           if(!is.null(old.loc)) { 
        						...
        						if(old.loc$left[1] < c.loc$right[1]) {
        											if(old.loc$left[2] < c.loc$right[2]) { 
                                          				dia.arrow(old.loc$center,c.loc$center,labels=labels,cex=cex,radius1=e.size,radius2=e.size)} else {
                                          dia.arrow(old.loc$center,c.loc$center,labels=labels,cex=cex,radius1=e.size,radius2=e.size)}} else {
          					 dia.arrow(old.loc$center,c.loc$center,labels=labels,cex=cex,radius1=e.size,radius2=e.size)}}
               ...
               if(size1==1) {
                ...
                dia.arrow(c.loc$center,v.loc$right,round(cluster["r1"],digits),cex=cex,radius1=e.size)
                  } else {
              		...
              		if(size2==1) {
              			...
              			dia.arrow(c.loc$center,v.loc$right,labels = round(clusters[head,"r2"],digits),cex=cex,radius1=e.size)
