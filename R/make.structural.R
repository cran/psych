"make.structural" <-
function (s=NULL,f=NULL,n=0,raw=FALSE) {
 require(MASS)
 #first, do the default case
 if(is.null(s)) { 
          s=diag(1,nrow=3)
          s[1,3] <- s[3,1] <- s[2,3] <- s[3,2] <- sqrt(.5)}
 
 if(is.null(f)) {f <-matrix(c(               
            .9,0,0,
            .8,0,.0,
            .6,.6,.0,
             0,.8,.0,
  ...
  model <-  f %*% s %*%  t(f) #the model correlation matrix for oblique factors
