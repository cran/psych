...
if (!is.null(yz)) {
       ...
       p <- pt(abs(t2),n-2,lower.tail=FALSE)
        if(twotailed) p <- 2*p
       return(list(test="test of difference between two correlated  correlations",t=t2,p=p))
       } else {
        ...
        z <- abs(diff/se.diff.r)
         p <- (1-pnorm(z ))
          if(twotailed) p <- 2*p
        return(list(test="test of difference between two independent correlations",z=z,p=p))
     	}
     	}
