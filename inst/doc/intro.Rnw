...
setCor(y = c( "y1", "y2"), x = c("x1","x2"), data = myData)
\end{Sinput}
\end{Schunk}
\end{scriptsize}
...
\begin{Schunk}
\begin{Sinput}
mediate(y = c( "y1", "y2"), x = c("x1","x2"), m= "med1", data = myData)
\end{Sinput}
\end{Schunk}
\end{scriptsize}
...
\begin{Schunk}
\begin{Sinput}
mediate(y = c( "y1", "y2"), x = c("x1","x2"), m= "med1", mod = "mod1",  data = myData)
\end{Sinput}
\end{Schunk}
\end{scriptsize}
\end{itemize}
...
Many additional functions, particularly designed for basic and advanced psychometrics are discussed more fully 
 in the Overview Vignette. A brief review of the functions available is included here.  In addition, there are helpful tutorials for \emph{Finding omega}, \emph{How to score scales and find reliability}, and for \emph{Using psych for factor analysis} at \url{http://personality-project.org/r}.
...
\label{sect:read}
Although many find copying the data to the clipboard and then using the \pfun{read.clipboard} functions (see below), a helpful alternative is to read the data in directly.  This can be done using the \pfun{read.file} function which calls \fun{file.choose} to find the file and then based upon the suffix of the file, chooses the appropriate way to read it.  For files with suffixes of .txt, .text, .r, .rds, .rda, .csv, .xpt, or  .sav,  the file will be read correctly. 
...
\end{Schunk} 
 ...
 \begin{Schunk}
\begin{Sinput}
 my.data <- read.file(widths = c(4,rep(1,35))  #will read in a file without a header row and 36 fields, the first of which is 4 colums, the rest of which are 1 column each.
\end{Sinput}
\end{Schunk} 
...
\begin{Schunk}
\begin{Sinput}
 my.spss  <- read.file(use.value.labels=TRUE)  #this will keep the value labels for .sav files
\end{Sinput}
\end{Schunk} 
...
\begin{Schunk}
\begin{Sinput}
mediate(y = c( "SATV" ), x = c("education" ,"age" ), m= "ACT", data =sat.act,std=TRUE,n.iter=50)
\end{Sinput}
\end{Schunk}
\end{scriptsize}


 \item \pfun{mediate}  will take raw data and find  (and graph the path diagram) a moderated multiple regression model  for multiple y variables depending upon multiple x variables mediated through a mediation variable. It then tests the mediation effect using a boot strap.  The particular example is for demonstration purposes only and shows neither moderation nor mediation.  The number of iterations for the boot strap was set to 50 for speed.  The default number of boot straps is 5000.
...
@
\end{scriptsize}
\caption{Moderated multiple regression requires the raw data.}
\label{default}
\end{center}
\end{figure}
...
\begin{Schunk}
\begin{Sinput}
> news(Version > "1.7.0",package="psych")
\end{Sinput}
\end{Schunk}
