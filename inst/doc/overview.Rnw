...
Because of the difficulty of installing the  package \Rpkg{Rgraphviz}, alternative graphics have been developed and are available as ``diagram" functions.   If \Rpkg{Rgraphviz} is available, some functions will take advantage of it.
...
sa.mat <- describe.by(sat.act,list(sat.act$gender,sat.act$education),skew=FALSE,ranges=FALSE,mat=TRUE)
...
@
\caption{The basic graphic capabilities of the dia functions}
\label{fig:dia}
\end{center}
\end{figure}
