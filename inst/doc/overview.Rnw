...
Some of the functions described in this overview require other packages.  Particularly useful for rotating the results of factor analyses (from e.g.,  \pfun{fa}, \pfun{factor.minres}, \pfun{factor.pa}, \pfun{factor.wls}, or  \pfun {principal}) or hierarchical factor models using \pfun{omega} or \pfun{schmid}, is the \Rpkg{GPArotation} package.  For analyzing correlations of dichotomous data, the \Rpkg{polychor} package is required in order to use either the \pfun{poly.mat} or \pfun{phi2poly} functions. Although sometimes hard to install, the \Rpkg{Rgraphviz} package is well worth the effort in order to appreciate the structure of hierarchical factor models or hierarchical cluster models.  (Other useful packages can be installed by installing and then using the task views (\Rpkg{ctv}) package to install the ``Psychometrics" task view, but are not necessary.)
...
\item[\pfun{fa}] incorporates three alternative algorithms: \iemph{minres factor analysis}, \iemph{principal axis factor analysis} and \iemph{weighted least squares factor analysis}. That is, it includes the functionality of three other functions that will be eventually phased out.
...
The maximum likelihood solution to this equation is found by \fun{factanal} in the \Rpkg{stats} package. Three alternatives are provided in \Rpkg{psych}, all of them are included in the \pfun{fa} function and are called by specifying the factor method (e.g., fm=``minres", fm=``pa", or fm=``"wls"). 
