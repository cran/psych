...
The maximum likelihood solution to this equation is found by \fun{factanal} in the \Rpkg{stats} package.  An alternative, least squares algorithm, \pfun{factor.pa}, does a  Principal Axis factor analysis by iteratively doing an eigen value decomposition of the correlation matrix with the diagonal replaced by the values estimated by the factors of the previous iteration.  This OLS solution is not as sensitive to improper matrices as is the maximum likelihood method, and will sometimes produce more interpretable results. 

A classic data set, collected by \cite{thurstone:41} and then reanalyzed by \cite{bechtoldt:61} and discussed by \cite{mcdonald:tt}, is a set of 9 cognitive variables with a clear bi-factor structure \cite{holzinger:37}.  Three factors were requested for this solution (Table~\ref{tab:Thurstone}).
\begin{table}[htdp]
...
factor.plot(f3p)
@
\caption{A graphic representation of the 3 factors from the Thurstone data using \pfun{factor.plot}}
\label{fig:thurstone}
\end{center}
\end{figure}
...
<<print=FALSE,echo=TRUE>>=
factor.congruence(list(om,p3p,f3p))
@
