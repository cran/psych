...
round(bifact,2)
@
These data can be represented as either a \iemph{bifactor} (Figure~\ref{fig:bifact} panel A) or \iemph{hierarchical} (Figure~\ref{fig:bifact} Panel B) factor solution.  The analysis was done with the \pfun{omega} function.  The graphs require the \Rpkg{Rgraphviz} package. 
\begin{figure}[htbp]
\begin{center}
...
f3 <-fa(bifact.s,3)     #extract three factors
