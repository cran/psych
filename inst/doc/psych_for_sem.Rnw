...
\item[sim.anova] Simulate a 3 way balanced ANOVA or linear model, with or without repeated measures.
\end{description}
...
An alternative model is to extract three factors and try this solution.  The \pfun{fa} factor analysis function (using the \iemph{minimum residual} algorithm) is used to detect the structure.  Alternatively, the \fun{factanal} could have  been used.
...
f3 <-fa(bifact.s,3)     #extract three factors and do an oblique rotation
...
Comparing the two models graphically (Figure~\ref{fig:omega:cfa} with Figure~\ref{fig:thurstone:bi} shows that while not identical, they are very similar.  The sem version is basically a forced simple structure. Notice that the values of $\omega_h$ are not identical from the EFA and CFA models. 
