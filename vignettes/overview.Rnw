...
By specifying the number of subjects in correlation matrix, appropriate estimates of standard errors, t-values, and probabilities are also found.  The next example finds the regressions with variables 1 and 2 used as covariates.  The $\beta$ weights for variables 3 and 4 do not change, but the multiple correlation is much less.  It also shows how to find the residual correlations between variables 5-9 with variables 1-4 removed.
...
Cluster analysis, factor analysis, and principal components analysis all produce structure matrices (matrices of correlations between the dimensions and the variables) that can in turn be compared in terms of the \iemph{congruence coefficient}  which is just the  cosine of the angle between the dimensions.  Consider the case of a four factor solution and four cluster solution to the Big Five problem.  
...
Surprisingly, 107 years after \cite{spearman:rho} introduced the concept of reliability to psychologists, there are still multiple approaches for measuring it. Although very popular, Cronbach's  $\alpha$  \citep{cronbach:51} underestimates the reliability of a test and over estimates the first factor saturation \citep{rz:09}.
...
\subsection{Reliability of multiple scales within an inventory}
...
Once the items have been scored as true or false (assigned scores of 1 or 0), they made then be scored into multiple scales using the normal \pfun{score.items} function. 
...
For example, taking the output from the 14 ability items from the \iemph{SAPA} project when scored for True/False using \pfun{score.multiple.choice} we can first do a simple IRT analysis of one factor  (Figure~\ref{fig:iq.irt}) and then use that correlation matrix to do an  \pfun{omega} analysis to show the sub-structure of the ability items .  
...
om <- omega(iq.irt$rho)
@
\caption{An Omega analysis  of 14 ability items sampled from the SAPA project.  The items represent a general factor as well as some lower level factors.  In particular, items 43, 44 and47 represent abstract analogical ability. The analysis is done using the tetrachoric correlations found in the previous \pfun{irt.fa} analysis. }
\label{fig:iq.irt}
\end{center}
\end{figure}
...
\item [\pfun{super.matrix}] Takes two matrices A and B  and combines them into a ``Super matrix'' with A on the top left, B on the lower right, and 0s for the other  two quadrants. A useful trick when forming complex keys, or when forming example problems.
 \end{description}
...
\begin{Schunk}
\begin{Sinput}
> news(Version > "1.1.10",package="psych")
\end{Sinput}
\end{Schunk}
...
\bibitem[\protect\astroncite{Fox et~al.}{2011}]{sem}
Fox, J., Byrnes, J., with contributions~from Michael~Culbertson, Friendly, M.,
  Kramer, A., and Monette., G. (2011).
...
\newblock R package version 1.1.12.
...
Revelle, W., Condon, D.~M., and Wilt, J. (2011).
\newblock Methodological advances in differential psychology.
\newblock In Tomas Chamorro-Premuzic, A. F.~F. and von Stumm, S., editors, {\em
  Handbook of Individual Differences}, chapter~2, pages 39--73.
  Wiley-Blackwell.
